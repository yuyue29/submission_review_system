...
public class ReviewerDaoImpl implements ReviewerDao {
	...
	public Reviewer save(Reviewer reviewer) {
		if (findBy("id", reviewer.getId()).size() != 0) {
			String sql = "UPDATE `submission`.`reviewer` t SET t.`account_` = ?, t.`password_` = ?, t.`name_` = ?, t.`contact` = ?, t.`total` = ? WHERE t.`id` LIKE ? ESCAPE '#'";
		...
		String sql = "INSERT INTO `submission`.`reviewer` (`id`, `account_`, `password_`, `name_`, `contact`, `total`) VALUES (?, ?, ?, ?, ?, ?)";
    ...
    public Reviewer delete(Reviewer reviewer) {
		String sql = "DELETE FROM `submission`.`reviewer` WHERE `id` LIKE ? ESCAPE '#'";
    ...
    public List<Reviewer> findAll() {
		String sql = "SELECT t.* FROM `submission`.`reviewer` t LIMIT 501";
    ...
    public List<Reviewer> findBy(String type, String value) {
		String sql = "SELECT t.* FROM `submission`.`reviewer` t WHERE `" + type + "` = ? LIMIT 501";

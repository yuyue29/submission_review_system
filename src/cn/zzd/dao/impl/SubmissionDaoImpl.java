...
public class SubmissionDaoImpl implements SubmissionDao {
	...
	public Submission save(Submission submission) {
		...
		if (findBy("id", submission.getId()).size() != 0) {
			String sql = "UPDATE `submission`.`submission` t SET t.`name_` = ?, t.`content` = ?, t.`state` = ? WHERE t.`id` LIKE ? ESCAPE '#'";

			template.update(sql, submission.getName_(), submission.getContent(), submission.getState(), submission.getId());
			return submission;
		}
		String sql = "INSERT INTO `submission`.`submission` (`id`,`name_`, `content`, `state`) VALUES (?, ?, ?, ?)";
	...
	public Submission delete(Submission submission) {
		String sql = "DELETE FROM `submission`.`submission` WHERE `id` LIKE ? ESCAPE '#'";
	...
	public List<Submission> findAll() {
		String sql = "SELECT t.* FROM `submission`.`submission` t LIMIT 501";
    ...
    public List<Submission> findBy(String type, String value)  {
		String sql = "SELECT t.* FROM `submission`.`submission` t WHERE `" + type + "` = ?  LIMIT 501";

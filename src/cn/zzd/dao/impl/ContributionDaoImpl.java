...
public class ContributionDaoImpl implements ContributionDao {
	...
	public Contribution save(Contribution contribution) {
		if (findBy("id", contribution.getId()).size() != 0) {
			String sql = "UPDATE `submission`.`author_submission_mapping` t SET t.`author_id` = ?, t.`submission_id` = ? WHERE t.`id` LIKE ? ESCAPE '#'";
			template.update(sql,
					contribution.getAuthorId(),
					contribution.getSubmissionId(),contribution.getId());
			return contribution;
		}
		String sql = "INSERT INTO `submission`.`author_submission_mapping` (`id`, `author_id`, `submission_id`) VALUES (?, ?, ?)";
	...
	public Contribution delete(Contribution contribution) {
		String sql = "DELETE FROM `submission`.`author_submission_mapping` WHERE `id` LIKE ? ESCAPE '#'";
	...
	public List<Contribution> findAll() {
		String sql = "SELECT t.* FROM `submission`.`author_submission_mapping` t LIMIT 501";
	...
	public List<Contribution> findBy(String type, String value) {
		String sql = "SELECT t.* FROM `submission`.`author_submission_mapping` t WHERE `" + type + "` = ? LIMIT 501";
	...
	public List<Contribution> findUni(String value1, String value2) {
		String sql = "SELECT t.* FROM submission.author_submission_mapping t WHERE author_id=? AND submission_id=? LIMIT 501";

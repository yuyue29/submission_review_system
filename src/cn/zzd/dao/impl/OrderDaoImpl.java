...
public class OrderDaoImpl implements OrderDao {
    ...
    public Order save(Order order) {
		if (findBy("id", order.getId()).size() != 0) {
			String sql = "UPDATE `submission`.`review_submission_mapping` t SET t.`state` = ?, t.`payment` = ?, t.`fee` = ?, t.`charge` = ?, t.`reviewer_id` = ?, t.`submission_id` = ? WHERE t.`id` LIKE ? ESCAPE '#'";
		...
		String sql = "INSERT INTO `submission`.`review_submission_mapping` (`id`, `state`, `payment`, `fee`, `charge`, `reviewer_id`, `submission_id`) VALUES (?, ? , ?, ?, ?, ?, ?)";
    ...
    public Order delete(Order author) {
		String sql = "DELETE FROM `submission`.`review_submission_mapping` WHERE `id` LIKE ? ESCAPE '#'";
    ...
    public List<Order> findAll()  {
		String sql = "SELECT t.* FROM `submission`.`review_submission_mapping` t LIMIT 501";
		List<Order> authors;
    ...
    public List<Order> findBy(String type, String value)  {
		String sql = "SELECT t.* FROM `submission`.`review_submission_mapping` t WHERE `" + type + "` = ? LIMIT 501";
